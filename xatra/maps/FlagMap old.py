...
class Map():
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            flags (List[Flag]): List of Flags.
            geojson (DataCollection | List[Dict], optional): List of features (GeoJSON dicts)
                or DataCollection container for a list of features to use as the Map base.
            geojson_rivers (DataCollection | List[Dict]): List of rivers (GeoJSON dicts) or DataCollection container for a list of rivers to be shown.
                Defaults to [].
        ...
        self.flags = flags
        if isinstance(geojson, DataCollection):
            self.geojson = geojson.load(verbose=verbose)
        else:
            self.geojson = geojson
        if isinstance(geojson_rivers, DataCollection):
            self.geojson_rivers = geojson_rivers.load(verbose=verbose)
        else:
            self.geojson_rivers = geojson_rivers
        self.custom_colors = custom_colors
        
        if verbose:
            print(f"Map: Calculating options")
        ...
        if verbose:
            print(f"Map: Calculating cache properties")
    ...
    def _geojson_with_flags(self, year=None):
        ...
        """
        if self.verbose:
            print(f"Map: Calculating geojson with flags for year {year}")
        result = []
        for feature in self.geojson:
            if self.verbose:
                print(f"Map: Adding flags to feature {NAME_max(feature)['NAME_n']}")
            feature_copies = []
            matching_flags = [
                flag.name
                for flag in self.flags
    ...
    def _calc_geojson_by_year(self):
        ...
        """
        if self.verbose:
            print(f"Map: Stacking _geojson_with_flags for breakpoint years")
        if self.breakpoints:
    ...
    def _calc_color_map(self):
        ...
        if self.verbose:
            print(f"Map: Calculated color map")
    ...
    def plot(self, path_out=None):
        ...
        if self.geojson_rivers:
            ...
            for river in self.geojson_rivers:
                if self.verbose:
                    print(f"Map: Adding river {river['properties']['river_name']}")
                rivers.add_child(
                    folium.GeoJson(
                        data=river,
    ...
    def plot_flags_as_layers(self, show_by_default=None, path_out=None):
        ...
        for flag in self.flags:
            if flag.name in self._unique_flag_names:  # relevant flags only
                if self.verbose:
                    print(f"Map: Adding layer for flag {flag.name}")
